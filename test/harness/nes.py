...
#       （sample_phases）。
#
#   (b) 作業変数 — act_timer / act_hitstop / act_invuln のようなタイマー類、
#       fx_life（効果の寿命）、oam_next / oam_used / sort_count のような組み立て中の数
...
#       そこでなら観測点の位置に依存しない。
#
#       実例（レーン時代。ADR-0009 で機構ごと無くなったが、作法の理由として残す）:
#       「レーン移動の完了後に補間の作業変数が片付いている」は、到着処理
#       （dec ent_lane_step → 足元Yの吸着 → acc/dy の消去）の**途中**、dec の直後の
#       分岐で観測していた。step だけ 0 で dy はまだ 20 のままの点である。
#       エンジンは正しく、テストが覗く位置が悪かった。
#       src/action/ が入ってメインループが伸びた日に落ちた。
#       **レーンの補間状態機械が消えて主要因は減ったが、作法そのものは要る。**
#       act_timer / act_hitstop / fx_life は同じ性質の量である（ADR-0009 §帰結）。
