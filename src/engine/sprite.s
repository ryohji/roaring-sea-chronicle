...
;   3. 影                 … 有効なエンティティの足元
;
; 影を最後に置いたのは ADR-0002 の「どれを捨てるか」の帰結である。影は奥行きを
...
; 影のタイルを1枚ずつ敷く。
;
; **これが擬似奥行きを「規則」ではなく「空間」に見せる唯一の手がかりである。**
; 影が無いと、16ドット高のキャラが縦にずれて並ぶだけの絵になり、
; 縦に動いたことが「奥へ動いた」に見えない（P1 受入での主の指摘）。
; 足元Y（ent_y）に敷くので、奥行きを連続にしても（ADR-0009）そのまま連続に動く。
;
