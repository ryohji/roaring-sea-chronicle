...
ADR-0001（セーブ方式）／0002（敵同時数）／0003（操作キャラ切替）は決定済み。
現在の未解決は **ADR-0005 の「後続の検討事項」のうち2件**であり、いずれも勝手に決めてはならない。
番号は ADR-0005 の採番のまま据え置く（振り直すと他の文書からの参照が壊れる）。

1. 蘇生手段の出所 — **研究3系統に4本目を足すことは禁止**（企画書の三本柱に直結）。
   ADR-0006 により優先度が上がった。猶予中はプレイヤーが何もできないため、
   **蘇生手段が無ければ猶予は結果を眺めるだけの時間になる**
2. ~~猶予中の操作~~ → **ADR-0006 で決定済み**（キャラ操作は無効、メタ操作は可能）
