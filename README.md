...
ADR-0005 の「後続の検討事項」のうち2件。**勝手に決めて実装しない。**
番号は ADR-0005 の採番のまま（2番は [ADR-0006](docs/decisions/0006-input-while-downed.md) で決定済み）。

1. **蘇生手段の出所** — アイテム／庇護型の固有能力／客将の固有必殺／用意しない。
   **研究3系統（響／錠／灼）に4本目を足すことは禁止**（企画書の三本柱に直結するため）。
   [ADR-0006](docs/decisions/0006-input-while-downed.md) により優先度が上がった。
   猶予中はプレイヤーが何も操作できないため、**蘇生手段が無ければ、操作キャラの猶予は
   結果を眺めるだけの時間になる**。その場合は猶予を短くするか、猶予なしで即シナリオ失敗とする方が誠実。
